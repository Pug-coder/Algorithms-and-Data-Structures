...
class MFA:
        ...
        for dtype, dec in declarations:
            if dtype == 'node':
                ...
                if symbol.isdigit():
                    ...
                    assert symbol == 'eps' or symbol == 'any' or len(symbol) == 1

                self.nodes.add(node_id1)
                self.nodes.add(node_id2)

                self.edges.add(
                    (node_id1, node_id2, symbol, tuple(memory))
                )
    ...
    def find_traps(self):
        ...
        [network.add_edge(self.numbering[x[0]], self.numbering[x[1]]) for x in self.edges]
        ...
        for key, value in connection_num.items():
            ...
            nums.add(key)
            connection[self.rev_numbering[key]] = set(map(lambda x: self.rev_numbering[x], nums))
        self.traps = set()
        for n in self.nodes:
    ...
    def normalize_edges(self):
        ...
        for node in self.nodes:
            ...
            to_pop = [key if not value else None for key, value in groups.items()]
            ...
            for destination in groups:
                ...
                flags_to_alpha = dict([(y, set()) for _, y in groups[destination]])
                ...
                for key_combination, alphas_set in flags_to_alpha.items():
                    ...
                    if alphas_set == self.alphabeth:
                        ...
                        s = set(map(lambda alpha: (node, destination, alpha, key_combination), self.alphabeth))
                        print(s)
                        self.edges = self.edges.difference(s)
                        self.edges.add((node, destination, 'any', key_combination))
    ...
    def detect_edges_flags(self):
        ...
        for node in self.nodes:
            ...
            if len(transits) == 1:
                ...
                continue
            if any(filter(lambda x: x[2] == 'any' or x[2] == 'eps' or x[2].isdigit(), transits)):
                continue
            for alpha in self.alphabeth:
        ...
        for e in self.edges:
            ...
            for x in e[3]:
                print(x, 'c', x == 'c')
        self.writings = set(filter(lambda edge: any(map(lambda x: x != 'c', edge[3])), self.edges))
        self.memory_independend = set(filter(lambda edge: not edge[2].isdigit(), self.edges))
